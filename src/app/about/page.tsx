...
import { CheckCircle2, Target, Eye, Heart, Shield, Lightbulb, Star, Zap, User } from "lucide-react"
import { useTeam } from "@/lib/useStore"

const values = [
  { icon: Shield,    title: "HONORABILITY",       desc: "Conducting business with integrity, honesty, and ethical principles." },
  { icon: Star,      title: "SERVICE EXCELLENCE", desc: "Striving for excellence in service to healthcare professionals and institutions." },
  { icon: Lightbulb, title: "MINDFUL INNOVATION", desc: "Embracing continuous improvement and innovative solutions." },
  { icon: Zap,       title: "EFFICIENCY",         desc: "Operating with efficiency to streamline the procurement process." },
  { icon: Heart,     title: "SATISFACTION",       desc: "Prioritizing customer satisfaction through quality products and service." },
]
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-6 animate-in slide-in-from-left-12 duration-1000 fill-mode-both">
              ...
              <div className="space-y-6 text-white/90 leading-relaxed text-lg">
                <p>Healthsync Medical Solutions Corporation is a trusted partner in the world of medical equipment supply.</p>
      ...
      <section className="py-24 bg-muted/50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12 max-w-6xl mx-auto">
            {team.map((member, i) => (
              <div key={member.id} className="text-center space-y-4 animate-in slide-in-from-bottom-8 duration-700 fill-mode-both" style={{ animationDelay: `${i * 150}ms` }}>
                <div className="relative aspect-square rounded-2xl overflow-hidden shadow-md mx-auto max-w-[250px] group bg-muted">
                  {member.photo ? (
                    <Image
                      src={member.photo}
                      alt={member.name}
